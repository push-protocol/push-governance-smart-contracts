...
const {
  fetechFutureContract,
} = require("../../scripts/utils/fetechFutureContract");
...
const TOKENS = [
  { Token: "EPNSMock", mode: "blocknumber" },
  // { Token: '$ERC20VotesTimestampMock', mode: 'timestamp' },
  // { Token: '$ERC20VotesLegacyMock', mode: 'blocknumber' },
];

const name = "PushGovernor";
const version = "1";
...
async function deployToken(contractName) {
  ...
  try {
    return await ethers.deployContract(contractName, [owner.address]);
  } catch (error) {
...
describe("Governor", function () {
  for (const { Token, mode } of TOKENS) {
    const fixture = async () => {
      ...
      const token = await deployToken(Token, owner.address);
      const factory = await ethers.getContractFactory("PushGovernor");
      const pushTimelockController = await ethers.getContractFactory(
        "PushTimelockController"
      );
      const timelock_address = await fetechFutureContract(owner, 1);
      const governance_address = await fetechFutureContract(owner, 3);
      ...
      const timelockController = await upgrades.deployProxy(
        pushTimelockController,
        [0n, proposers, executors, timelock_address],
        { initializer: "initialize" }
      );
      const mock = await upgrades.deployProxy(factory, [
        token.target,
        timelockController.target, //dummy timelock
        votingDelay,
        votingPeriod,
        0n,
      ]);
    ...
    describe(`using ${Token}`, function () {
      ...
      it("deployment check", async function () {
        ...
        expect(await this.mock.quorum(0)).to.equal(ethers.parseEther("4"));
      ...
      it.skip("send ethers", async function () {
      ...
      describe("vote with signature", function () {
        ...
        it.skip("votes with a valid EIP-1271 signature", async function () {
          const factory = await ethers.getContractFactory("ERC1271WalletMockUpgradeable");
          const wallet = await upgrades.deployProxy(factory, [
            this.userEOA,
          ],{ initializer: false });
        ...
        afterEach("no other votes are cast", async function () {
          expect(
            await this.mock.hasVoted(this.proposal.id, this.owner)
          ).to.be.false;
          expect(
            await this.mock.hasVoted(this.proposal.id, this.voter1)
          ).to.be.false;
          expect(
      ...
      describe("should revert", function () {
        describe("on propose", function () {
          ...
          it.skip("if proposer has below threshold votes", async function () {
        ...
        describe("on execute", function () {
          ...
          it.skip("if receiver revert without reason", async function () {
            this.helper.setProposal(
              [
                {
                  target: this.receiver.target,
          ...
          it.skip("if receiver revert with reason", async function () {
            this.helper.setProposal(
              [
                {
                  target: this.receiver.target,
      ...
      describe("cancel", function () {
        describe("internal", function () {
          ...
          it.skip("after vote", async function () {
            await this.helper.propose();
            await this.helper.waitForSnapshot();
            await this.helper
              .connect(this.voter1)
          ...
          it.skip("after deadline", async function () {
            await this.helper.propose();
            await this.helper.waitForSnapshot();
            await this.helper
              .connect(this.voter1)
          ...
          it.skip("after execution", async function () {
            await this.helper.propose();
            await this.helper.waitForSnapshot();
            await this.helper
              .connect(this.voter1)
      ...
      //         "NFT",
      //       ]);
      //       await this.token.mint(this.owner, tokenId);
      //     });

      //     it.skip("can receive an ERC721 safeTransfer", async function () {
      //       await this.token
      //         .connect(this.owner)
      ...
      //   });
        ...
        //   it.skip("can receive ERC1155 safeTransfer", async function () {
        ...
        //   it.skip("can receive ERC1155 safeBatchTransfer", async function () {
        //     await this.token
        //       .connect(this.owner)
