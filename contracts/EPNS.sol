...
contract EPNS {
    ...
    function approve(
        ...
        if (rawAmount >= type(uint96).max) {
            ...
            require(
                amount == rawAmount,
                "Push::approve: amount exceeds 96 bits"
            );
        }
    ...
    function permit(
        ...
        if (rawAmount == type(uint256).max) {
            ...
            require(
                ...
                "Push::permit: amount exceeds 96 bits"
            );
            amount = uint96(rawAmount);
        }
    ...
    function transfer(address dst, uint256 rawAmount) external returns (bool) {
        uint96 amount = safe96(
            rawAmount,
            "Push::transfer: amount exceeds 96 bits"
        );
     ...
     */
    function returnHolderDelegation(
        address account,
        address delegate
    ...
    function resetHolderWeight(address holder) external {
        require(
            holderDelegation[holder][msg.sender] == true ||
                holder == msg.sender,
    ...
    function burn(uint256 rawAmount) external {
        ...
        balances[account] = sub96(
            balance,
            amount,
            "Push::burn: burn amount exceeds balance"
        );
        totalSupply = sub256(
            totalSupply,
            rawAmount,
            "Push::burn: supply underflow"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "Push::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "Push::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "Push::delegateBySig: signature expired"
        );
    ...
    function getPriorVotes(
        ...
        require(
            ...
            "Push::getPriorVotes: not yet determined"
        );
        ...
        Checkpoint memory lastCheckpoint = checkpoints[account][
            nCheckpoints - 1
        ];
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(
            src != address(0),
            "Push::_transferTokens: cannot transfer from the zero address"
        );
        require(
            dst != address(0),
            "Push::_transferTokens: cannot transfer to the zero address"
        );
        ...
        balances[src] = sub96(
            balances[src],
            amount,
            "Push::_transferTokens: transfer amount exceeds balance"
        );
        balances[dst] = add96(
            balances[dst],
            amount,
            "Push::_transferTokens: transfer amount overflows"
        );
    ...
    function _adjustHolderWeight(
        ...
        if (balance == 0) {
            ...
            uint256 dstWeight = mul256(
                holderWeight[dst],
                balance,
                "Push::_adjustHolderWeight: holder dst weight exceeded limit"
            );
            uint256 srcWeight = mul256(
                holderWeight[src],
                amount,
                "Push::_adjustHolderWeight: holder src weight exceeded limit"
            );

            uint256 totalWeight = add256(
                dstWeight,
                srcWeight,
                "Push::_adjustHolderWeight: total weight exceeded limit"
            );
            uint256 totalAmount = add256(
                balance,
                amount,
                "Push::_adjustHolderWeight: total amount exceeded limit"
            );

            uint256 totalAmountBy2 = div256(totalAmount, 2, "");
            uint256 roundUpWeight = add256(
                totalWeight,
                totalAmountBy2,
                "Push::_adjustHolderWeight: round up amount exceeded limit"
            );

            holderWeight[dst] = div256(
                roundUpWeight,
                totalAmount,
                "Push::_adjustHolderWeight: adjusted holder negative divide"
            );
        }
    }
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(
                    srcRepOld,
                    amount,
                    "Push::_moveVotes: vote amount underflows"
                );
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(
                    dstRepOld,
                    amount,
                    "Push::_moveVotes: vote amount overflows"
                );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "Push::_writeCheckpoint: block number exceeds 32 bits"
        );
